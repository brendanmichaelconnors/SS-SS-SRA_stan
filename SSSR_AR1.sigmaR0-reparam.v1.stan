...
parameters{
  ...
  real<lower=0> sigma_R0_raw; // process error for first a.max years with no spawner link
  ...
  real<lower=0> tau; // for reparameterization of sigma_R0

}

transformed parameters{
  real<lower=0> sigma_R0; // reparameterized to be non-centered; see:https://mc-stan.org/docs/2_18/stan-users-guide/reparameterization-section.html
  ...
  real half_nu;// for reparameterization of sigma_R0


  R = exp(lnR);

  sigma_R0 = sigma_R0_raw/sqrt(tau);
  half_nu = 0.5 * 1;
...
model{
  ...
  sigma_R0_raw ~ std_normal(); // implies sigma_R0 ~ student_t(1, 0, 5);
