...
// BC: good points, have removed bounds on both parameters
parameters{
...
model{
  ...
  // mean_ln_R0 ~ normal(0,10000); // SA: reduce scale? BC: yeah makes sense, a SD of 20 on log scale is still plenty variation for prior
  mean_ln_R0 ~ normal(0,20); // SA
  // sigma_R0 ~ inv_gamma(0.1,0.1); // SA: avoid if possible; see other .stan file BC: was vestidual apendage of JAGS model but point taken, instead of student-t could also just go with half normal?
  sigma_R0 ~ student_t(1, 0, 5); // BC: made more heavy tailed with larger sigma than SA suggestion (student_t(7, 0, 3)
  //sigma_R0 ~ normal(0,5);

  // SA: vectorize?
  for (i in 1:a_max){
    lnR[i] ~ normal(mean_ln_R0, sigma_R0);
  }
  ...
  // Observation model
  for(t in 1:nyrs){
    //U[t] ~ uniform(0.01, 0.99); // SA: not needed; use Beta or normal? BC: Ok, have removed for now since it is bounded above anyway
